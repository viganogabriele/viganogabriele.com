...
import { ArrowUpRight, Mail, Menu, Power, X } from "lucide-react";
  ...
  return <>
    <m.nav ref={navRef} aria-label="Primary navigation" initial={reduced ? false : { y: -36, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.7, ease: ease.cinematic, delay: 0.12 }} className="safe-nav fixed left-1/2 top-3 z-[60] w-[calc(100%-1.25rem)] max-w-6xl -translate-x-1/2 sm:top-6 sm:w-[calc(100%-2rem)]">
      ...
      <AnimatePresence>{open && <m.div id="mobile-navigation" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: reduced ? 0 : dur.micro }} className="mt-2 border border-white/[0.09] bg-surface/98 p-2 lg:hidden">{navItems.map((item) => item.href.startsWith("#")
              ...
              : <Link key={item.href} to={item.href} onClick={() => setOpen(false)} className="flex min-h-12 items-center gap-2 px-3 font-mono text-[10px] uppercase tracking-[0.14em] text-zinc-400 hover:bg-white/[0.04] hover:text-white">{item.label}<ArrowUpRight aria-hidden="true" className="h-3 w-3 opacity-70" /></Link>)}
              {/* Contact is a full row of its own rather than a squeezed extra
                  button: same 48px target as every other entry, and it keeps the
                  bar's own Contact button — hidden below sm — reachable on a
                  phone. Last in the panel so it does not displace navigation. */}
              <a href={`mailto:${profile.email}`} onClick={() => setOpen(false)} className="mt-1 flex min-h-12 items-center gap-2 border-t border-white/[0.08] px-3 font-mono text-[10px] uppercase tracking-[0.14em] text-accent hover:bg-white/[0.04] hover:text-white"><Mail aria-hidden="true" className="h-3.5 w-3.5" />Contact</a></m.div>}</AnimatePresence>
    </m.nav>
  </>;
}
