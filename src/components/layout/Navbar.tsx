...
export function Navbar({ onNavigate, systemActive, onToggleSystem }: { onNavigate: (target: string) => void; systemActive: boolean; onToggleSystem: () => void }) {
  ...
  const glass = isTelegramWebView || level === "lite" ? "rgba(5,6,8,0.94)" : scrolled ? "rgba(5,6,8,0.74)" : "rgba(5,6,8,0.48)";
  return <>
    <motion.nav ref={navRef} initial={reduced ? false : { y: -36, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.7, ease: ease.cinematic, delay: 0.12 }} className="safe-nav fixed left-1/2 top-3 z-[60] w-[calc(100%-1.25rem)] max-w-6xl -translate-x-1/2 sm:top-6 sm:w-[calc(100%-2rem)]">
      <div className="border border-white/[0.09] px-2 py-1.5 sm:px-4 sm:py-2" style={{ background: glass, backdropFilter: isTelegramWebView || level === "lite" ? "none" : "blur(22px)", WebkitBackdropFilter: isTelegramWebView || level === "lite" ? "none" : "blur(22px)" }}>
        <div className="flex items-center justify-between gap-3">
          <button type="button" onClick={(event) => go(event, "body")} data-cursor="hover" className="flex h-11 w-11 items-center justify-center focus-visible:outline focus-visible:outline-2 focus-visible:outline-cyan-200" aria-label="Back to top"><img src={logo} alt="Gabriele Viganò" width="160" height="134" className="h-5 w-auto invert" /></button>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2"><div className="relative"><AnimatePresence>{systemActive && level === "full" && <motion.span key="ring" className="pointer-events-none absolute inset-[-3px] rounded-sm border border-cyan-400/70" initial={{ scale: 1, opacity: 0.7 }} animate={{ scale: 1.55, opacity: 0 }} transition={{ duration: 1.1, repeat: Infinity, ease: "easeOut" }} />}</AnimatePresence><button type="button" onClick={onToggleSystem} data-cursor="hover" className={cn("nav-sys-btn relative inline-flex h-11 min-w-11 items-center justify-center gap-2 border px-2.5 font-mono text-[9px] uppercase tracking-[0.13em] transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-cyan-200", systemActive ? "border-cyan-400/60 text-cyan-400 hover:border-cyan-300 hover:text-cyan-300" : "border-white/[0.1] text-zinc-300 hover:border-cyan-200/50 hover:text-cyan-100")} aria-pressed={systemActive} aria-label="Toggle system mode"><Power className={cn("h-3 w-3 transition-colors", systemActive ? "text-cyan-400" : "")} /><span className="hidden sm:inline">SYS</span></button></div><a href="mailto:info@viganogabriele.com" data-cursor="hover" className="hidden min-h-11 items-center bg-[#f2f3f5] px-3 text-[10px] font-semibold text-black sm:inline-flex">Let’s talk</a><button ref={menuButtonRef} type="button" onClick={() => setOpen((value) => !value)} className="inline-flex h-11 w-11 items-center justify-center border border-white/[0.1] text-zinc-200 focus-visible:outline focus-visible:outline-2 focus-visible:outline-cyan-200 lg:hidden" aria-label="Toggle navigation" aria-controls="mobile-navigation" aria-expanded={open}>{open ? <X className="h-4 w-4" /> : <Menu className="h-4 w-4" />}</button></div>
        </div>
      </div>
      <AnimatePresence>{open && <motion.div id="mobile-navigation" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: reduced ? 0 : dur.micro }} className="mt-2 border border-white/[0.09] bg-[#080a0e]/98 p-2 lg:hidden">{navItems.map((item) => <a key={item.href} href={item.href} onClick={(event) => go(event, item.href)} className="flex min-h-12 items-center px-3 font-mono text-[10px] uppercase tracking-[0.14em] text-zinc-400 hover:bg-white/[0.04] hover:text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-cyan-200">{item.label}</a>)}</motion.div>}</AnimatePresence>
    </motion.nav>
  </>;
}
