 ...
 * the carousel. This writes CSS custom properties through a ref instead, the way
 * CircularCarousel and CustomCursor already drive their transforms.
 */
export function BorderGlow({ sensitivity = 0.34, coneSpread = 26 }: { sensitivity?: number; coneSpread?: number }) {
  ...
  usePointerFrames({
    ...
    target: () => layer.current?.parentElement ?? null,
    onPoint: (point) => {
      ...
      node.style.setProperty("--glow-angle", `${((Math.atan2(dy, dx) * 180) / Math.PI + 90).toFixed(2)}deg`);
